...
{
  // 动态获取
    private static string DataTableDirectoryPath => OutputDataTableDirectoryPath;
    private static string CSharpCodeDirectoryPath => OutputCSharpCodeDirectoryPath;
    private static string CSharpCodeTemplateFilePath => InputCSharpCodeTemplateFilePath;
    ...
    {
        return new DataTableProcessor(Utility.Path.GetRegularPath(Path.Combine(DataTableDirectoryPath, dataTableName + ".txt")), Encoding.GetEncoding("GB2312"), 1, 2, null, 3, 4, 1);
    }
    ...
    {
        string binaryDataFileName = Utility.Path.GetRegularPath(Path.Combine(DataTableDirectoryPath, dataTableName + ".bytes"));
    ...
    public static void GenerateCodeFile(DataTableProcessor dataTableProcessor, string dataTableName)
    {
        ...
        string csharpCodeFileName = Utility.Path.GetRegularPath(Path.Combine(CSharpCodeDirectoryPath, "DR" + dataTableName + ".cs"));
    ...
    private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
    {
        ...
        //暂时修改 ==== > 
        codeContent.Replace("__DATA_TABLE_CREATE_TIME__", string.Empty);
        // codeContent.Replace("__DATA_TABLE_CREATE_TIME__", DateTime.UtcNow.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"));
        codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "Table");
        codeContent.Replace("__DATA_TABLE_CLASS_NAME__", "DR" + dataTableName);
        codeContent.Replace("__DATA_TABLE_COMMENT__", dataTableProcessor.DataTableDescription); // 设置数据表名备注
